export default function templateTaskReportManager(data: {
  managerName: string;
  reportDate: string;
  tasks: Array<{
    dueDate: string;
...
<head>
    ...
    <title>Relatório Geral de Tarefas - ${data.reportDate}</title>
</head>
                        ...
                        <td style="padding: 40px;">
                            <div style="margin-bottom: 25px;">
                                <h2 style="color: #333333; margin: 0 0 10px 0; font-size: 18px;">
                                  Olá, ${data.managerName}
                                </h2>
                                <p style="color: #666666; line-height: 1.6; margin: 10px 0;">
                                  Segue o relatório geral de <strong>${data.totalTasks} tarefa(s) pendente(s)</strong> da equipe.
                                </p>
                            </div>
                            ...
                            <div style="margin-top: 30px; padding: 20px; background-color: #f8f9fa; border-radius: 6px;">
                              <p style="color: #666666; margin: 0; font-size: 14px; line-height: 1.6;">
                                <strong>Total de tarefas pendentes:</strong> ${data.totalTasks}<br>
                                <strong>Tarefas vencidas:</strong> ${data.tasks.filter((t) => t.isOverdue).length}
                              </p>
                            </div>
                        </td>
                    </tr>
